RSpec.describe 'inject pattern test' do
  ...
  it 'test 6' do
    ...
    toppings = []
    topping_calories.each do |item,price|
      toppings.append(item.to_s)
    end
  ...
  it 'test 8' do
    ...
    total_calories = 0
    cost = 1
    toppings.each do |item, qunat|  #item here is the key and qunat is hash, so we need to loop over the hash and not the key
      result = 1.0
      qunat.each do |x,y|
        result *= y
      end
  ...
  it 'test 9' do
    ...
    final_grade = 0
    grades.each do |topic,grade|
      sum = 0.0
      grade.each do |g|
        sum += g
      end
      final_grade += (sum / grade.length).to_f
    end
